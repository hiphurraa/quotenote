		...
		try:
			for a in text.findAll('a'):
				if ("/wiki/" in a.get('href')):
					...
					peopleNames.append(a.text)
		except:
			print("Ongelmia tekstissa")
	time.sleep(0.1)

numberOfPeople = len(peopleLinks)
peopleIndices = [1847,2255,2752,3573,3944,4001,4763,5001,5489,5687,6019,6991,7089,7917,7942,10400,11737]

#file1 = open('wikis.txt', 'w')
...
for peopleIndex in peopleIndices:
	...
	try:
		...
		for index in finalIndexes:
			...
			sql = "INSERT INTO skeema.quote(user_id, quotetext, said_by) VALUES (1, '" + quoteText[:endIndex] + "', '" + peopleNames[peopleIndex] + "')"
			try:
				...
				#print("ei voitu kirjoittaa quotea")
				conn.rollback()
